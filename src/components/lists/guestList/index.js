...
const GuestList = ({recept, navigation, onChange, ActivityOn}) => {
  ...
  return (
    <View style={styles.list}>
      ...
      {entries.length === 0 && !ActivityState && !ActivityOn && (
      ...
      {entries.length >= 1 &&
        (!ActivityState || !ActivityOn) &&
        entries.filtered('isCheckin == false').length === 0 && (
          <View style={styles.listext}>
            <Text style={styles.text}>
              TODOS OS CONVIDADOS JÁ DERÃO CHECK-IN
            </Text>
          </View>
        )}
      {entries.length >= 1 &&
        ...
        entries.filtered('isCheckin == false').length !== 0 && (
          <FlatList
            data={
              recept.name !== '' && recept.name !== undefined
                ? entries
                    .filtered(
                      'name LIKE [c] $0 AND isCheckin == false',
                      recept.name.toUpperCase() + '*',
                    )
                    .sorted('name')
                : entries.filtered('isCheckin == false').sorted('name')
            }
              ...
              <View
                ...
                <TouchableOpacity
                  onPress={() => {
                    Alert.alert(
                      'Atenção',
                      `Tem certeza que deseja dar "Check-In" em ${item.name}?`,
                      [
                        {
                          text: 'Cancel',
                          onPress: () => {},
